...
//data
assign data_sram_en = ex_mem_en;//(ex_mem_en & ~ex_mem_wr) | (wb_mem_en & MEM_ready_go & WB_allowin & MEM_valid);
assign data_sram_wen = ex_mem_wen;//(wb_mem_en & MEM_valid) ? wb_mem_wen : 4'b0;
assign data_sram_addr = ex_mem_paddr;//(wb_mem_en & MEM_valid) ? wb_mem_paddr : ex_mem_paddr;
assign data_sram_wdata = ex_mem_wdata;//(wb_mem_en & MEM_valid) ? wb_mem_wdata : ex_mem_wdata;
assign data_sram_rsize = ex_mem_rsize;
assign dcache_v = ex_dcache_v;//(wb_mem_en & MEM_valid) ? wb_dcache_v : ex_dcache_v;

wire wr_buf_wating;
assign wr_buf_wating = 1'b0;//wb_mem_en & MEM_valid & ex_ctrl_en[2] & EXE_valid & ~ex_mem_wr;
