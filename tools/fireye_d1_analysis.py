#!/usr/bin/env python3
"""
nscscc_perf/fireye_D1 性能瓶颈分析
- trace: /tmp/fireye_d1/simu_trace.txt
- 区间: get_count start(行259051) ~ stop(行2277356)
- 反汇编: /tmp/fireye_d1/fireye_d1.dis
"""
import re
...
insn_info = {}
for line in open("/tmp/fireye_d1/fireye_d1.dis"):
...
# ---------- 解析 trace (计时区间) ----------
START_LINE, END_LINE = 259051, 2277356
def parse(path):
    groups = []
    cur_ns = None; cur = []
    with open(path) as f:
...
for g in groups:
    for pc, inst in g[1]:
        func[(int(pc,16) >> 12) & 0xff] += 1
print(f"\n=== 热点代码区 (0x1c00XXxx) ===")
for k, v in func.most_common(8):
    print(f"  0x1c00{k:02x}xx: {v} ({v/N*100:.1f}%)")
